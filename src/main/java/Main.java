...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 1; i <= 9; i++) {
            String filename = "data/generated/input" + i + ".json";
            System.out.println("\n==== Processing " + filename + " ====");
            ...
            // Topological sort
            List<Integer> topo = TopologicalSortKahn.sort(dag);
            System.out.println("Topo order of SCCs: " + topo);

            // Shortest paths
            int[] dist = DAGSP.shortestPath(dag, topo, 0);
            System.out.print("Shortest distances from source 0: ");
            for (int d : dist) System.out.print(d + " ");
            System.out.println();

            // Longest path
            LongestPathDAG.Result lp = LongestPathDAG.longestPath(dag, topo, 0);
            System.out.println("Longest path length: " + lp.length);
            System.out.println("Longest path: " + lp.path);
        }
